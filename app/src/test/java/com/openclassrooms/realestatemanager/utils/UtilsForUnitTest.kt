...
class UtilsForUnitTest {
    companion object {
        ...
        val QUERY_STRING_FULL = " SELECT * FROM real_estate WHERE rooms >= ? AND price >= ? AND price <= ? AND creationDate >= ? AND sellDate >= ? AND surface >= ? AND surface <= ? AND bathrooms >= ? AND bedrooms >= ? AND pictureListSize >= ? AND address LIKE ? AND pointOfInterest LIKE ? AND pointOfInterest LIKE ? AND state LIKE ?"
